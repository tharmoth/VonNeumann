...
func move_camera(delta):
	...
	if mouse_near_left(threshold) or Input.is_action_pressed("LEFT"):
		new_target_velocity.x = -step
	elif mouse_near_right(threshold) or Input.is_action_pressed("RIGHT"):
		new_target_velocity.x = step

	if mouse_near_top(threshold) or Input.is_action_pressed("UP"):
		new_target_velocity.y = -step
	elif mouse_near_bottom(threshold) or Input.is_action_pressed("DOWN"):
		new_target_velocity.y = step
